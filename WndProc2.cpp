...
LRESULT CALLBACK WndProc2(HWND hwnd, UINT message, WPARAM wParam, LPARAM lParam)
{
	...
	PAINTSTRUCT ps;
	RECT        rect;
	tagWNDPROC2 * wnd2 = (tagWNDPROC2*)(GetWindowLongPtr(hwnd, 0));
	...
	case WM_PAINT: //what to do when a paint msg occurs.  This message is forced when the windows is "dirty" so it repaints
		...
		GetClientRect(hwnd, &rect); //define drawing area for 
		...
		SetTextColor(hdc, wnd2->colour);
		DrawText(hdc, wnd2->msg, -1, &rect,		//-1 where to end the message
